//9) app/(app)/settings/options/_components/modals/ProductRecommendationsModal.tsx
"use client";
...
export default function ProductRecommendationsModal({
  ...
  const handleSave = async () => {
    try {
      ...
      console.error("Failed to save product_recommendations", error);
      setSaveState("error");
  ...
  return (
    <OptionsModalShell
      open={open}
      onClose={onClose}
      title="تخصيص خيار منتجات ربما تعجبك"
      footer={
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <select
            value={form.type}
            onChange={(e) =>
              setForm((p) => ({ ...p, type: e.target.value as Value["type"] }))
            }
            className="h-11 w-full rounded-2xl border border-slate-200 px-4 outline-none"
          >
            <option value="random">منتجات عشوائية</option>
            <option value="category">منتجات من نفس التصنيف</option>
            <option value="brand">منتجات من نفس الماركة</option>
            <option value="tag">منتجات من نفس الوسم</option>
          </select>
        </div>

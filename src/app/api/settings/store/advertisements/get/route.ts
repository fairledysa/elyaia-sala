// FILE: apps/merchant/src/app/api/settings/store/advertisements/get/route.ts
...
function normalizeAdvertisementsFromHeaderAndFooter(
  headerAndFooter: Record<string, any>,
): AdvertisementSettings[] {
  const rawList = Array.isArray(headerAndFooter.advertisements)
    ? headerAndFooter.advertisements
    : Array.isArray(headerAndFooter.items)
      ? headerAndFooter.items
      : [];
  ...
  const legacy = headerAndFooter.announcement ?? headerAndFooter.advertisement;
  if (!legacy) return [];
...
export async function GET() {
  try {
    ...
    const row = await getThemeOptionsRow(storeId, versionId);

    const themeOptions = safeObject(row?.value);
    const headerAndFooter = safeObject(themeOptions.header_and_footer);

    const advertisements = normalizeAdvertisementsFromHeaderAndFooter(
      headerAndFooter,
    );
    ...
    return NextResponse.json(
      {
        ...
        meta: {
          ...
          setting_slug: themeOptionsSlug(versionId),

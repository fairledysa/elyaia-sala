						...
						<div className='flex gap-2'>
							{row.original.category.map((item) => (
								<Badge key={item.id} variant='soft' color='zinc'>
									<div className={`h-4 w-1 rounded-2xl bg-${item.color}-500`}></div>
									<span>{item.name}</span>
								</Badge>
							))}
						</div>
					...
					cell: ({ row }) => (
						<div className='flex gap-2'>
							{row.original.store.map((item) => (
								<Badge key={item} variant='outline' color='zinc' className='truncate'>
									{item}
								</Badge>
							))}
						</div>
			...
			cell: ({ row }) => (
				<div className='flex gap-2'>
					<Button
						...
						onClick={() =>
							router.push(
								`${pages.apps.products.subPages.edit.to}?productId=${row.original.id}`,
							)
						}
					/>
				</div>

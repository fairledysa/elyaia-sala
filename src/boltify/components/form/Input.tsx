...
export interface IInputBaseProps
	...
	// ✅ props تنمرر بالغلط من فوق – لا لازم تروح للـ DOM
	rightIcon?: unknown;
	leftIcon?: unknown;
	isActive?: unknown;
	isactive?: unknown;
...
const Input = forwardRef<HTMLInputElement, TInputProps>((props, ref) => {
	...
	// ✅ يطلع ثابت بين SSR/CSR
	const reactId = useId();
	const stableId = useMemo(() => id || `inp-${reactId}`, [id, reactId]);
	...
	return (
		<>
			{label && !isFloatingLabel && <Label htmlFor={stableId}>{label}</Label>}
			{_INPUT}
		</>
	);
});

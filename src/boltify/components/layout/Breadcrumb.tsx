...
const Breadcrumb: FC<IBreadcrumbProps> = (props) => {
	const { className, list, homePath = '/' } = props;
	...
	return (
		<div
			...
			<Dropdown className='inline-flex! md:hidden!'>
				...
				<DropdownMenu>
					<DropdownItem icon='Home09' onClick={() => router.push(homePath)}>
						Home
					</DropdownItem>
					{list.map((item) => {
						if (item.to) {
			...
			{list.map((item, idx) => {
				...
				const itemClassName = classNames(baseClass, responsiveClass);

				if (item.to) {
					return (
						<Link key={item.text} className={itemClassName} href={item.to}>
				...
				return (
					<div key={item.text} className={itemClassName}>
						{item.icon && <Icon icon={item.icon} />}
						<div className='truncate max-md:max-w-32'>{item.text}</div>
					</div>
				);

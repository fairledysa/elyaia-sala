...
const normalizePath = (p?: string | null) => {
	...
	if (!noQuery.startsWith('/')) return `/${noQuery}`.replace(/\/+/g, '/');
	return noQuery.replace(/\/+/g, '/');
};

const isRouteActive = (pathname: string, to: string, end = true) => {
	const current = normalizePath(pathname);
	const target = normalizePath(to);

	// نفس سلوك NavLink end: true
	if (end) return current === target;

	// لو احتجت لاحقًا (صفحات فرعية)
...
export const NavItem: FC<INavItemProps> = (props) => {
	...
	const activeByRoute = to ? isRouteActive(pathname, to, true) : false;
	const isActiveFinal = Boolean(isActiveOverwrite || activeByRoute);
	...
	const desktopLinkClass = classNames(
		navItemClasses.default,
		isActiveFinal ? navItemClasses.active : navItemClasses.inactive,
		'max-md:hidden',
	);

	const mobileLinkClass = classNames(
		navItemClasses.default,
		isActiveFinal ? navItemClasses.active : navItemClasses.inactive,
		'md:hidden',
	);
		...
		<Tooltip text={asideStatus ? '' : t(text)} placement='right'>
			<li
				...
				{to ? (
					<>
						{/* For Desktop */}
						<Link href={to} className={desktopLinkClass}>
							{CONTENT}
						</Link>
...
export const NavCollapse: FC<INavCollapseProps> = (props) => {
	...
	const pathname = usePathname();
	const here = normalizePath(to) !== '/' && normalizePath(pathname).includes(normalizePath(to));
...
export const NavUser: FC<INavUserProps> = (props) => {
	...
	const desktopLinkClass = classNames(
		navItemClasses.default,
		activeByRoute ? navItemClasses.active : navItemClasses.inactive,
		'max-md:hidden',
	);
			...
			<li
				...
				{to ? (
					<>
						...
						{/* For Mobile */}
						<Link href={to} onClick={() => setAsideStatus(false)} className={mobileLinkClass}>
							{CONTENT}
						</Link>
					</>
				) : (
